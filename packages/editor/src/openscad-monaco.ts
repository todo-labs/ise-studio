...
export function installOpenSCADMonaco(monacoInstance: typeof Monaco) {
  ...
  registerOpenSCADNavigation(monacoInstance);
}
...
function registerOpenSCADNavigation(monacoInstance: typeof Monaco) {
  monacoInstance.languages.registerDefinitionProvider("openscad", {
    provideDefinition: (model, position) => {
      const word = model.getWordAtPosition(position);
      if (!word) return undefined;
      const pattern = new RegExp(`\\bmodule\\s+${escapeRegExp(word.word)}\\s*\\(`);
      for (let line = 1; line <= model.getLineCount(); line += 1) {
        const content = model.getLineContent(line);
        const match = pattern.exec(content);
        if (match) {
          const startColumn = match.index + match[0].indexOf(word.word) + 1;
          return {
            uri: model.uri,
            range: {
              ...
              startColumn,
              endLineNumber: line,
              endColumn: startColumn + word.word.length,
            },
          };
        }
      }
  ...
  monacoInstance.languages.registerReferenceProvider("openscad", {
    provideReferences: (model, position) => {
      const word = model.getWordAtPosition(position);
      if (!word) return [];
      return model.findMatches(`\\b${escapeRegExp(word.word)}\\b`, false, true, false, null, false).map((match) => ({
        uri: model.uri,
        range: match.range,
      }));
    },
