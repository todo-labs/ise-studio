When changing project files or persistence, start in `packages/studio-project/src`.
When changing OpenSCAD compile behavior, start in `packages/studio-openscad/src`.
When changing preview rendering, start in `apps/studio/src/features/preview`.
When changing Monaco editor behavior, start in `packages/studio-editor/src` and `apps/studio/src/features/editor`.
When changing assistant tools or prompts, start in `packages/studio-ai/src`.
